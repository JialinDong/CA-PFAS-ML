...
def get_index(df,j):
    ...
    for tail_label in tail_labels:
        sub_index = set(df[df[tail_label] == 1].index)   # change 0 to 1 for sub3
        if i == 0:
...
def main( ):
    ...
    if file_name == 'pfas_MLSMOTE':
        ...
        # Load Data
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_3_super.csv")  # 0
    ...
    # binary models
        ...
        # 找出含有空值的行
        df_semi = pd.read_csv("GW_MAIN_PFAS_14_subset_3_all.csv")       # 3
    ...
    # get the Pseudo labels and train the ml again (split the train/test, then balance, then used the train_p=label to train ml)
        ...
        # 找出含有空值的行
        df_semi = pd.read_csv("GW_MAIN_PFAS_14_subset_3_all.csv")       # 0
        df_semi = df_semi.drop(columns=['gm_gis_dwr_region'])  # 'latitude', 'longitude',
        df_semi = df_semi[df_semi.isnull().T.any()]
        ...
        df_semi_data_only.to_csv('GW_MAIN_PFAS_14_subset_3_SemiONLY.csv', index=False)  # save file
        ...
        df_semi_add.to_csv('GW_MAIN_PFAS_14_subset_3_SemiAll.csv', index=False)  # save file


        df_semi_data_only = pd.read_csv("GW_MAIN_PFAS_14_subset_3_SemiONLY.csv")
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_3_super.csv")
        df = df.drop(columns=['gm_gis_dwr_region'])  # 'latitude', 'longitude',
        ...
        for i in range(y_res.shape[1]):
            ...
            print( i, list(y_res.columns.values)[i], ': ', counter, 'rate:' , rate)
        ## repeat the process until the dataset is balanced
        X_sub, y_sub = get_minority_instace(X_res, y_res, 1)  # Getting minority instance of that dataframe
        X_res_1, y_res_1 = MLSMOTE(X_res, y_res, X_sub, y_sub, 2000)  # Applying MLSMOTE to augment the dataframe
        # summarize class distribution
        print('oversampling dataset 2')
        for i in range(y_res_1.shape[1]):
        ...
        X_sub, y_sub = get_minority_instace(X_res_1, y_res_1, 1)  # Getting minority instance of that dataframe
        X_res_2, y_res_2 = MLSMOTE(X_res_1, y_res_1, X_sub, y_sub, 2000)  # Applying MLSMOTE to augment the dataframe
        # summarize class distribution
        print('oversampling dataset 3')
        for i in range(y_res_2.shape[1]):
        ...
        X_sub, y_sub = get_minority_instace(X_res_3, y_res_3, 1)  # Getting minority instance of that dataframe
        X, y = MLSMOTE(X_res_3, y_res_3, X_sub, y_sub, 2000)  # Applying MLSMOTE to augment the dataframe
        # summarize class distribution
        print('oversampling dataset 5 -- change > ')
        for i in range(y.shape[1]):
            counter = Counter(y.iloc[:, i])
        ...
        # oversampling - duplicated the get_minority_instace
        # X_sub, y_sub = get_minority_instace(X_res_4, y_res_4, 1)  # Getting minority instance of that dataframe
        ...
        print("y6: %.4f, y7: %.4f" % (auc_y6, auc_y7))
        print("ROC AUC total ave: ", auc_total)
        ...
        print(cm_y1, cm_y2, cm_y3, cm_y4, cm_y5,cm_y6, cm_y7)
        ...
        print(cr_y1, cr_y2, cr_y3, cr_y4, cr_y5,cr_y6, cr_y7)
    ...
    # balance then split to train, test
        ...
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_3_SemiAll.csv")
