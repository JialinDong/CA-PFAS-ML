...
def main( ):
    ...
    if file_name == 'pfas_MLSMOTE':
        ...
        # Load Data
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_2_super.csv")  # 0
    ...
    # binary models
        ...
        # 找出含有空值的行
        df_semi = pd.read_csv("GW_MAIN_PFAS_14_subset_2_all.csv")       # 0
    ...
    # get the Pseudo labels and train the ml again (split the train/test, then balance, then used the train_p=label to train ml)
        ...
        df_semi_data_only.to_csv('GW_MAIN_PFAS_14_subset_2_SemiONLY.csv', index=False)  # save file
        ...
        df_semi_add.to_csv('GW_MAIN_PFAS_14_subset_2_SemiAll.csv', index=False)  # save file


        df_semi_data_only = pd.read_csv("GW_MAIN_PFAS_14_subset_2_SemiONLY.csv")
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_2_super.csv")
        df = df.drop(columns=['gm_gis_dwr_region'])  # 'latitude', 'longitude',
        ...
        auc_y9 = roc_auc_score(ytest[:, 8], yhat[:, 8].toarray())
        auc_y10 = roc_auc_score(ytest[:, 9], yhat[:, 9].toarray())
        auc_total = np.mean([auc_y1, auc_y2, auc_y3, auc_y4, auc_y5, auc_y6, auc_y7, auc_y8, auc_y9, auc_y10])
        print("ROC AUC y1: %.4f, y2: %.4f, y3: %.4f, y4: %.4f, y5: %.4f" % (auc_y1, auc_y2, auc_y3, auc_y4, auc_y5))
        print("y6: %.4f, y7: %.4f, y8: %.4f, y9: %.4f, y10: %.4f" % (auc_y6, auc_y7, auc_y8, auc_y9, auc_y10))
        print("ROC AUC total ave: ", auc_total)
        ...
        print(cm_y1, cm_y2, cm_y3, cm_y4, cm_y5,cm_y6, cm_y7, cm_y8, cm_y9, cm_y10)
        ...
        print(cr_y1, cr_y2, cr_y3, cr_y4, cr_y5,cr_y6, cr_y7, cr_y8, cr_y9, cr_y10)
    ...
    # balance then split to train, test
        ...
        # 找出含有空值的行
        df_semi = pd.read_csv("GW_MAIN_PFAS_14_subset_2_all.csv")       # 2
        df_semi = df_semi.drop(columns=['gm_gis_dwr_region'])  # 'latitude', 'longitude',
        df_semi = df_semi[df_semi.isnull().T.any()]
        df_semi = df_semi.reset_index(drop=True)
        ...
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_2_SemiAll.csv")
