...
def config_parser():
    ...
    parser.add_argument("--which_data", type=str, default='pfas_Pseudo_labeling_XBG',help='pfas_Pseudo_binary_XBG,pfas,pfas_MLSMOTE，example')
...
def main( ):
    ...
    if file_name == 'example_mlsmote':
        print('example dataset -- make')

        X, y = create_dataset()  # Creating a Dataframe
        for i in range(y.shape[1]):
            counter = Counter(y.iloc[:,i])
            print('class',i,': ',counter)

        X_sub, y_sub = get_minority_instace(X, y)  # Getting minority instance of that dataframe
        X_res, y_res = MLSMOTE(X, y, X_sub, y_sub, 500)  # Applying MLSMOTE to augment the dataframe
        # summarize class distribution
        for i in range(y_res.shape[1]):
            counter = Counter(y_res.iloc[:,i])
            print('class',i,': ',counter)
        ## repeat the process until the dataset is balanced
        X_sub, y_sub = get_minority_instace(X_res, y_res)  # Getting minority instance of that dataframe
        X_res_1, y_res_1 = MLSMOTE(X_res, y_res, X_sub, y_sub, 800)  # Applying MLSMOTE to augment the dataframe
        ...
        X_sub, y_sub = get_minority_instace(X_res_1, y_res_1)  # Getting minority instance of that dataframe
        X_res_2, y_res_2 = MLSMOTE(X_res_1, y_res_1, X_sub, y_sub, 800)  # Applying MLSMOTE to augment the dataframe
        ...
        print('finished')
        # https://medium.com/thecyphy/handling-data-imbalance-in-multi-label-classification-mlsmote-531155416b87#id_token=eyJhbGciOiJSUzI1NiIsImtpZCI6IjI3Yjg2ZGM2OTM4ZGMzMjdiMjA0MzMzYTI1MGViYjQzYjMyZTRiM2MiLCJ0eXAiOiJKV1QifQ..oPLpNX9opdSG2HataE0IfVrYwZF3vlba_1E_zO3HNQ_B_qzQkGm1PLAKxHnE2u4HZFyrtqAby6X5lUNv6FMqF_j4u3RVTsd9Ege7cZB2ARq3lYGmOTEG-xoBULonJnYlV4Z5AT5YSG96hX3rV-JxKS4ejnkV5AIVYEiM-7xBC60XbHsTgHsgUtM4gKpCJZNaWUolTX5q5dBCOWbDn5LiZgJTnxvrsnhpjF-oncOZJ-SahA5nwayUfKvmDqqTGI4wWKLy1kM5ZOxZN-KY_boMDY0wglXoeBeAoQq8oYq6EW1v2ElazaLFz86Z_b16hQfuyVrN6xLNFgnEKHWduxCfpg

    elif file_name == 'pfas_MLSMOTE':
    ...
    elif file_name == 'pfas_Pseudo_1':
        print('pfas dataset -- pfas_Pseudo Labeling 1')
        '''
        # Load Data
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_0_super.csv")       # 0
        df = df.drop(columns=['gm_gis_dwr_region'])  # 'latitude', 'longitude',
        ...
        # 找出含有空值的行
        df_semi = pd.read_csv("GW_MAIN_PFAS_14_subset_0_all.csv")       # 0
    ...
    elif file_name == 'pfas_Pseudo_2':
        print('pfas_Pseudo Labeling 2')
        from sklearn.utils import shuffle
        from sklearn.base import BaseEstimator, RegressorMixin
        from xgboost.sklearn import XGBRegressor

        # over sampling - ignore
        # Semi-surperviose ML 伪标签 Pseudo Labeling - QDA
        ...
        # Load Data
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_0_super.csv")       # 0
        train = df.drop(columns=['gm_gis_dwr_region'])  # 'latitude', 'longitude',

        # 找出含有空值的行
        df_semi = pd.read_csv("GW_MAIN_PFAS_14_subset_0_all.csv")       # 0
        df_semi = df_semi.drop(columns=['gm_gis_dwr_region'])  # 'latitude', 'longitude',
        test = df_semi[df_semi.isnull().T.any()]

        print(train.shape, test.shape)
        # Preprocess the data
        features = train.columns[0:113]
        target = list(df.columns[113:])[0]     # later for loop
        ...
        print('finished training and test')

    elif file_name == 'pfas_Pseudo_3_example':
        print('pfas_Pseudo_3_example')
        ...
        N_TEST = 100 * N_TRAIN
        np.random.seed(1)
        x_train, y_train = func_create_data(N_TRAIN)    # Training Data: real data
        x_test, y_test = func_create_data(N_TEST)    # test data: Pseudo Labeling

        model = RandomForestClassifier(random_state=0).fit(x_train, y_train)
        y_pred = model.predict(x_test)
        print('Before PseudoLabelingF1 - Score:', f1_score(y_test, y_pred))
        ...
        model = RandomForestClassifier(random_state=0).fit(x_train, y_train)
        y_pred = model.predict(x_test)
        print('AfterPseudoLabelingF1 - Score:', f1_score(y_test, y_pred))

        print('finished')
        # https://flag-editors.medium.com/%E6%A9%9F%E5%99%A8%E5%AD%B8%E7%BF%92%E5%8B%95%E6%89%8B%E5%81%9Alesson-1-%E5%96%84%E7%94%A8pseudo-labeling%E5%A2%9E%E5%8A%A0%E8%A8%93%E7%B7%B4%E8%B3%87%E6%96%99%E9%9B%86-95d7d4617c4f
    ...
    ### final method -- k-fold cross validation
        ...
        df_semi_data_only.to_csv('GW_MAIN_PFAS_14_subset_0_SemiONLY.csv', index=False)  # save file
        ...
        df_semi_add.to_csv('GW_MAIN_PFAS_14_subset_0_SemiAll.csv', index=False)  # save file


        df_semi_data_only = pd.read_csv("GW_MAIN_PFAS_14_subset_0_SemiONLY.csv")
        ...
        # print('average acc' ,Accuracy(ytest, yhat))  # 0.52777
        # Hamming Score
        print('Hamming score: {0}'.format(hamming_score(ytest, yhat)))  # 0.5277
        ...
        # https://zhuanlan.zhihu.com/p/420436876
        ...
        auc_y14 = roc_auc_score(ytest[:, 13], yhat[:, 13].toarray())
        auc_total = np.mean(
            [auc_y1, auc_y2, auc_y3, auc_y4, auc_y5, auc_y6, auc_y7, auc_y8, auc_y9, auc_y10, auc_y11, auc_y12, auc_y13,
             auc_y14])
        print("ROC AUC y1: %.4f, y2: %.4f, y3: %.4f, y4: %.4f, y5: %.4f, y6: %.4f, y7: %.4f" % (auc_y1, auc_y2, auc_y3, auc_y4, auc_y5,auc_y6,auc_y7))
        print("ROC AUC y8: %.4f, y9: %.4f, y10: %.4f, y11: %.4f, y12: %.4f, y13: %.4f, y14: %.4f" % (auc_y8, auc_y9, auc_y10, auc_y11, auc_y12, auc_y13, auc_y14))
        print("ROC AUC total ave: ", auc_total)
    ...
    ### balance then split to train, test
        ...
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_0_SemiAll.csv")
        ...
        cm_y5 = confusion_matrix(ytest[:, 4], yhat[:, 4].toarray())
        cm_y6 = confusion_matrix(ytest[:, 5], yhat[:, 5].toarray())
        cm_y7 = confusion_matrix(ytest[:, 6], yhat[:, 6].toarray())
        cm_y8 = confusion_matrix(ytest[:, 7], yhat[:, 7].toarray())
        cm_y9 = confusion_matrix(ytest[:, 8], yhat[:, 8].toarray())
        cm_y10 = confusion_matrix(ytest[:, 9], yhat[:, 9].toarray())
        cm_y11 = confusion_matrix(ytest[:, 10], yhat[:, 10].toarray())
        cm_y12 = confusion_matrix(ytest[:, 11], yhat[:, 11].toarray())
        cm_y13 = confusion_matrix(ytest[:, 12], yhat[:, 12].toarray())
        cm_y14 = confusion_matrix(ytest[:, 13], yhat[:, 13].toarray())
        print(cm_y1, cm_y2, cm_y3, cm_y4, cm_y5,cm_y6, cm_y7, cm_y8, cm_y9, cm_y10, cm_y11, cm_y12, cm_y13, cm_y14)
        ...
        cr_y5 = classification_report(ytest[:, 4], yhat[:, 4].toarray())
        cr_y6 = classification_report(ytest[:, 5], yhat[:, 5].toarray())
        cr_y7 = classification_report(ytest[:, 6], yhat[:, 6].toarray())
        cr_y8 = classification_report(ytest[:, 7], yhat[:, 7].toarray())
        cr_y9 = classification_report(ytest[:, 8], yhat[:, 8].toarray())
        cr_y10 = classification_report(ytest[:, 9], yhat[:, 9].toarray())
        cr_y11 = classification_report(ytest[:, 10], yhat[:, 10].toarray())
        cr_y12 = classification_report(ytest[:, 11], yhat[:, 11].toarray())
        cr_y13 = classification_report(ytest[:, 12], yhat[:, 12].toarray())
        cr_y14 = classification_report(ytest[:, 13], yhat[:, 13].toarray())
        print(cr_y1, cr_y2, cr_y3, cr_y4, cr_y5,cr_y6, cr_y7, cr_y8, cr_y9, cr_y10, cr_y11, cr_y12, cr_y13, cr_y14)
    ...
    # no use
    elif file_name == 'pfas_Pseudo_multi-XBG':
        print('pfas_Pseudo_multi-XBG')
        # Load Data
        df = pd.read_csv("GW_MAIN_PFAS_14_subset_0_super.csv")       # 0
        df = df.drop(columns=['gm_gis_dwr_region'])  # 'latitude', 'longitude',
        ...
        X = df.iloc[:, 0:113].values
        y = df.iloc[:, 113:].values     # .values
        # oversampling -- later
        # split to train, test
        x_train, x_test_real, y_train, y_test_real = train_test_split(X, y, test_size=0.2, random_state=0)  # Training Data: real data # test data: Pseudo Labeling

        x_test = df_semi.iloc[:, 0:113].reset_index(drop=True)    # x_train -- semi-ml data

        TH = 0.98        # 某一筆測試資料屬於標籤0的機率
        np.random.seed(1)

        # model = RandomForestClassifier(random_state=0).fit(x_train, y_train)
        # model = XGBClassifier(use_label_encoder=False).fit(x_train, y_train)
        model = ClassifierChain(XGBClassifier(use_label_encoder=False)).fit(x_train, y_train)
        y_pred_real = model.predict(x_test_real)
        # print('Before PseudoLabelingF1 - Score:', f1_score(y_test_real, y_pred_real))
        print('Before PseudoLabeling', x_train.shape, y_train.shape)
        y_pred = model.predict(x_test)
        y_prob = model.predict_proba(x_test)
        ...
        model = ClassifierChain(XGBClassifier(use_label_encoder=False)).fit(x_train, y_train)
        y_pred_real = model.predict(x_test_real)
        print('after PseudoLabelingF1 - Score:', f1_score(y_test_real, y_pred_real))
        print('after PseudoLabeling', x_train.shape, y_train.shape)
