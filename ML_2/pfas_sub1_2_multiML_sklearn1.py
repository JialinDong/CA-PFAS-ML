...
def main( ):
    ...
    print(len(xtest))
    # binary rel
    classifier = MultiOutputClassifier(CatBoostClassifier(n_estimators=200, silent=True))  # , silent=False
    clf = Pipeline([('classify', classifier)])
    print(clf)
    ...
    print(len(xtest))
    # ClassifierChain
    classifier = ClassifierChain(CatBoostClassifier(n_estimators=200, silent=True))  #, silent=False
    clf = Pipeline([('classify', classifier)])
    print(clf)
    ...
    # try the model by making random dataset
    print(model_name)
    xtrain, xtest, ytrain, ytest = train_test_split(X, y, train_size=0.8, random_state=2)  # random_state=2 结果可以被复现
    print(len(xtest))
    # binary rel
